...
/**
 * After the Spring context is fully initialized, scans all singleton beans
 * for WeConnectorFactory instances (by class name) and injects the OTel
 * WebClient filter into any WebClient fields found via reflection.
 */
class WeConnectorWebClientInjector implements ApplicationListener<ContextRefreshedEvent> {

    private static final Logger log = LoggerFactory.getLogger(WeConnectorWebClientInjector.class);
    ...
    WeConnectorWebClientInjector(ExchangeFilterFunction filter) {
        this.filter = filter;
    }
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        for (String beanName : beanFactory.getBeanDefinitionNames()) {
            try {
                ...
                if (bean != null && WE_CONNECTOR_FACTORY_CLASS.equals(bean.getClass().getName())) {
                    log.info("[oteljaeger] Found WeConnectorFactory bean: {}", beanName);
        ...
        if (injected > 0) {
            log.info("[oteljaeger] Injected OTel filter into {} WeConnectorFactory instance(s)", injected);
        } else {
            log.warn("[oteljaeger] No WeConnectorFactory instances found or no WebClient fields to inject");
        }
    }
    ...
    private boolean injectFilterIntoObject(Object obj, String path) {
        ...
        try {
            for (Field field : obj.getClass().getDeclaredFields()) {
                ...
                log.debug("[oteljaeger] Field '{}' in {} = {}", field.getName(), path, fieldType);
                ...
                if (value instanceof WebClient) {
                    ...
                    log.info("[oteljaeger] Injected OTel filter into WebClient field '{}' in {}", field.getName(), path);
                    injected = true;
                    ...
                    for (java.util.Map.Entry<?, ?> entry : map.entrySet()) {
                        if (entry.getValue() instanceof WebClient) {
                            ...
                            log.info("[oteljaeger] Injected OTel filter into WebClient in map['{}'] in {}", entry.getKey(), path);
                            injected = true;
                    ...
                    for (Object item : (java.util.Collection<?>) value) {
                        if (item instanceof WebClient) {
                            log.info("[oteljaeger] Found WebClient in collection[{}] in {}", i, path);
                            injected = true;
            ...
            log.debug("[oteljaeger] Could not inject into {}: {}", path, e.getMessage());
        }
        return injected;
    }
}
